#!/bin/bash
# Finalize all remaining translations by completing the 3 multiline strings for each language
...
# All remaining translations with actual newlines
FINAL_TRANS = {
    'ar': {
...
for lang_code in FINAL_TRANS.keys():
    ...
    # Replace each unfinished translation
    for source_text, trans_text in FINAL_TRANS[lang_code].items():
        # Escape special regex characters
        source_escaped = re.escape(source_text)

        # Pattern: <source>...</source>\n      <translation type="unfinished" />
        pattern = f'(<source>{source_escaped}</source>\\s*<translation type="unfinished" />)'
        replacement = f'<source>{source_text}</source>\n      <translation>{trans_text}</translation>'
...
print(f"✅ Completed {completed} final translations")

# Verify
print("\n" + "=" * 80)
print("FINAL VERIFICATION - ALL LANGUAGES")
print("=" * 80)
